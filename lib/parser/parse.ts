...
export function parse(
    ...
    if (typeof sentence !== "undefined" && param.sentence) {
      ...
      if (!param.optional && typeof val === "undefined") {
        return createParsingError({
          reason: "This parameter is not optional",
          index: result.index,
          param,
        });
      }
      ...
      if (final.length === 0) {
        if (typeof param.default === "undefined" && !param.optional) {
          return createParsingError({
            reason: "This parameter is not optional.",
            index: result.index,
            param,
  ...
  function createParsingError(opts: {
    ...
    return {
      args: {},
      error:
        `\`\`\`Error for parameter ${getParameterString(
          opts.param
        )}\n${content}\n${" ".repeat(opts.index + 1)}^\`\`\`` + opts.reason,
    };
  }
}
